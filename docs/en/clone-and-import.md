...
`git clone https://github.com/ByteLegendQuest/java-hexadecimal-character`

A project directory named `java-hexadecimal-character` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-hexadecimal-character` directory which is created just now.
